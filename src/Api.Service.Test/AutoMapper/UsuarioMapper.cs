...
namespace src.Api.Service.Test.AutoMapper
{
    public class UsuarioMapper : BaseTesteService
    {
        ...
        public void E_Possivel_Mapear_os_modelos()
        {
            ...
            var listaDto = Mapper.Map<List<UserDTO>>(list);
            Assert.True(listaDto.Count() == list.Count());
            for (int i = 0; i < listaDto.Count(); i++)
            {
                Assert.Equal(listaDto[i].Id, list[i].Id);
                Assert.Equal(listaDto[i].Name, list[i].Name);
                Assert.Equal(listaDto[i].Email, list[i].Email);
                Assert.Equal(listaDto[i].CreateAt, list[i].CreateAt);
            }
